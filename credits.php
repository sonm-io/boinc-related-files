...
if ($key == 'XXXXXXXXX') {
    if (! filter_var($appid, FILTER_VALIDATE_INT)) {
        ...
        //$sql = "SELECT id, workunitid, userid, hostid, received_time, name, flops_estimate, granted_credit, address FROM result RIGHT JOIN eth_wallets ON result.userid = eth_wallets.user_id left join eth_payments on result.id = eth_payments.taskid WHERE result.appid = $appid AND server_state = 5 AND validate_state = 1 AND eth_payments.status <> 0";
    	$sql = "SELECT id, workunitid, userid, hostid, received_time, name, flops_estimate, granted_credit, address FROM result RIGHT JOIN eth_wallets ON result.userid = eth_wallets.user_id WHERE result.appid = $appid AND server_state = 5 AND validate_state = 1 and result.id not in (select taskid from eth_payments where eth_payments.status != 0) limit 10000";
